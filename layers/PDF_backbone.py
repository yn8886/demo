...
class PDF_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int,
        ...
        self.pred_list = [math.ceil(target_window / i) for i in self.period_list]
        ...
        self.enembedding = nn.ModuleList([nn.Sequential(
            EnEmbedding(d_model, p, dropout)
        ) for i, p in enumerate(self.period_list)
        ])
                    ...
                    EncoderLayer(
                        AttentionLayer(
                            FullAttention(False, factor, attention_dropout=dropout,
                                          output_attention=False), d_model, self.period_list[i], n_heads),
                        d_model,
                        dropout=dropout,
        ...
        ) for i, n in enumerate(self.pred_list)])
        ...
        self.projection = nn.Linear(target_window, target_window)
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        if self.wo_conv:
            ...
            for i, period in enumerate(self.period_list):
                ...
                loc = self.pad_layer[i][0](z).reshape(z.shape[0] * z.shape[1], -1, period)
                loc = self.conv[i](loc)
                loc = self.enembedding[i](loc)
                glo = self.pad_layer[i][1](glo)
                glo = self.embedding[i](glo.unsqueeze(-3))
                glo = rearrange(glo, "(b m) d n -> b m d n", b=z.shape[0]).contiguous()
                glo = self.backbone[i](glo).reshape(z.shape[0] * z.shape[1], -1, period)
                glo = self.enembedding[i](glo)
                res1.append(glo + loc)

            res2 = []
            for i, ts in enumerate(res1):
                x = ts[:, -1, :].reshape(z.shape[0], z.shape[1], -1)
                outs, _ = self.decoder[i](x)
                res2.append(torch.cat(outs, dim=-1)[..., :self.pred_len])

            res = torch.sum(torch.stack(res2, dim=-1), -1)


        # denorm
        z = self.projection(res)
        z = z.permute(0, 2, 1)
